/*
	提示词链路（Prompt Chaining）：
		核心：将复杂的任务目标解耦为顺序执行的模块化步骤。这种"分而治之"的策略能有效降低模型的认知负荷，通过中间过程的可控性，显著降低幻觉率并提升最终结果的准确性。
		应用场景：Dify、Coze、N8N 等低代码/无代码平台

	上下文工程（Context Engineering）：
		定义：在 Token 生成之前，系统性地构建各种信息，为 AI 提供一个完整的"决策环境"。
		核心理念：上下文的质量 > 模型本身的架构。只要环境给的信息够丰富、够精准，模型的表现就能大幅提升。
		组成要素：
			- Prompt Engineering：基础指令设计
			- RAG（检索增强生成）：主动查阅外部文档
			- Tools（工具调用）：获取实时数据（如日历、API）
			- State/History（状态与历史）：记住用户身份和对话历史
			- Memory（记忆）：长期存储的关键信息
			- Structured Outputs（结构化输出）：保证输出机器可读

	个人理解：
		提示词链路：将用户任务拆分为定义好的执行链路，逐步执行，提高模型准确率。
		上下文工程：通过 Prompt、Tools、RAG、Memory、结构化输出、State/History 等要素，让模型获得更多外部知识和上下文信息，提升决策质量。
*/
